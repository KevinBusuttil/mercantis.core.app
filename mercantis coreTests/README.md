...
| `CLIInstallParityTests.swift` | `AppRuntime/` | P2.3 — pins `AppInstaller.decodeManifest(from:)` / `validate(manifestData:)` / `install(manifestData:)`. Asserts CLI install via JSON data produces identical engine-visible state (apps row, doctypes, fields, sync_queue, expression indexes) to the in-process `install(_:)` call, and that `FieldDefinition` decodes leniently when scaffolded JSON omits the layout fields. |
