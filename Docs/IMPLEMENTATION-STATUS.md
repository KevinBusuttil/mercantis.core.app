...
- **Shipped (P2.2 / ADR-029, 2026-04-25)** — Cross-document `lookup("DocType", id, "field")` is recognised by the evaluator when constructed with a `DocumentLookupResolver`. `DocumentEngine` is the reference resolver; `CachingDocumentLookupResolver` wraps it with a read-through cache that subscribes to `DocumentSavedEvent` / `DocumentDeletedEvent` / `DocumentSubmittedEvent` / `DocumentCancelledEvent` and drops the affected `(docType, id)` entry on every write. `DocumentEngine.list`'s `whereExpression` runs through the engine's pre-wired `listExpressionEvaluator` so list filters can call `lookup(...)` and the join is memoized once per parent across the row scan. A per-evaluation `lookupBudget` (default 32) caps cross-document reads — excess calls throw `EvaluatorError.lookupBudgetExceeded`. Without an injected resolver every `lookup(...)` call still throws, so existing call sites that build a bare `ExpressionEvaluator()` stay sandboxed exactly as before.
...
- **Barcode field (W9 / ADR-035, 2026-04-29)** — `FieldType.barcode` persists as a plain `String`. `BarcodeField` adds a text field plus an iOS-only `AVFoundation` scan button/sheet; on macOS the scan button is hidden and manual entry remains available. `GenericFormView` renders the field natively, `ValidationPipeline` accepts `.string` values for the type, and `DocTypeBuilderView` maps the type to `barcode`.
